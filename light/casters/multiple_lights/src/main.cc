...
void render(GLFWwindow *w, vector<EntityPtr<TexturePtr>> &es) {
  ...
  do {
    ...
    glUniform3f(e->p->uniform("v_pos"), c.x, c.y, c.z);

    glUniform3f(e->p->uniform("m.specular"), 0.633f, 0.7278f, 0.633);
    glUniform1f(e->p->uniform("m.shininess"), 128 * 0.6f);

    glUniform3f(e->p->uniform("d.ambient"), 0.1f, 0.1f, 0.1f);
    glUniform3f(e->p->uniform("d.diffuse"), 0.8f, 0.8f, 0.8f);
    glUniform3f(e->p->uniform("d.specular"), 1.0f, 1.0f, 1.0f);
    glUniform3f(e->p->uniform("d.dir"), -0.2f, -1.0f, -0.3f);

    glUniform3f(e->p->uniform("p_lights[0].pos"), l.x, l.y, l.z);
    glUniform3f(e->p->uniform("p_lights[0].ambient"), la.x, la.y, la.z);
    glUniform3f(e->p->uniform("p_lights[0].diffuse"), ld.x, ld.y, ld.z);
    glUniform3f(e->p->uniform("p_lights[0].specular"), 1.0f, 1.0f, 1.0f);
    glUniform1f(e->p->uniform("p_lights[0].cst"), 1.0f);
    glUniform1f(e->p->uniform("p_lights[0].lin"), 0.09);
    glUniform1f(e->p->uniform("p_lights[0].quad"), 0.032);
